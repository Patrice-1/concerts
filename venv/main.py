...
cursor.execute("INSERT INTO bands (name, hometown) VALUES (?, ?)", ('Maroon five', 'Chicago'))
cursor.execute("INSERT INTO venues (title, city) VALUES (?, ?)", ('State Park Hotel', 'Washington'))
connection.commit()

# Fetch the band and venue details
band = cursor.execute("SELECT * FROM bands WHERE name = ?", ('The Vipers',)).fetchone()
venue = cursor.execute("SELECT * FROM venues WHERE title = ?", ('Chicago',)).fetchone()

# Create a new concert
band_play_in_venue(band['id'], venue['title'], '2018.03.05')
